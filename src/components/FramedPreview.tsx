    ...
    return (
      <div className="relative">
        ...
        <div className="absolute bottom-4 right-4 flex bg-black/60 backdrop-blur-sm rounded-sm overflow-hidden">
          <button onClick={() => setViewMode('raw')} className="px-3 py-1.5 text-[10px] tracking-wider uppercase text-white/90 bg-white/20">Image</button>
          <button onClick={() => setViewMode('framed')} className="px-3 py-1.5 text-[10px] tracking-wider uppercase text-white/50 hover:text-white/80 transition-colors">Framed</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="relative">
      <div
        ...
        style={{ background: 'linear-gradient(180deg, #f0ede6 0%, #e8e4dc 100%)', minHeight: '300px' }}
      >
        <div
          className="relative transition-all duration-500"
          style={{ padding: frameWidth, backgroundColor: hex, backgroundImage: frameTexture(hex), boxShadow: frameShadow, maxWidth: '85%' }}
        >
          <div className="absolute inset-0 pointer-events-none" style={{ boxShadow: innerShadow }} />
          <div
            className="transition-all duration-500"
            style={{
              padding: mountWidth,
              backgroundColor: showMount ? '#faf9f6' : 'transparent',
              boxShadow: showMount ? 'inset 0 0 0 1px rgba(0,0,0,0.04), inset 0 1px 3px rgba(0,0,0,0.06)' : 'none',
            }}
          >
      ...
      <div className="absolute bottom-4 right-4 flex bg-black/60 backdrop-blur-sm rounded-sm overflow-hidden">
        <button onClick={() => setViewMode('raw')} className="px-3 py-1.5 text-[10px] tracking-wider uppercase text-white/50 hover:text-white/80 transition-colors">Image</button>
        <button onClick={() => setViewMode('framed')} className="px-3 py-1.5 text-[10px] tracking-wider uppercase text-white/90 bg-white/20">Framed</button>
      </div>
    </div>
  );
}